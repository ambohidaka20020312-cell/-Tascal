...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!ageConfirmed) { setError("13歳以上であることを確認してください"); return; }
    if (!termsAccepted) { setError("利用規約とプライバシーポリシーへの同意が必要です"); return; }
    ...
    <div className="min-h-screen flex bg-[var(--bg-primary)]">
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 bg-[var(--bg-secondary)] border-r border-[var(--border)]">
        ...
        <ul className="space-y-3">
          {["クレジットカード不要で無料スタート", "毎朝AIが今日の最適プランを提案", "タスク超過をリアルタイムで検知・再計画"].map((item) => (
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-8 py-12">
          <div className="w-full max-w-sm">
            <h2 className="text-2xl font-light tracking-wide text-[var(--text-primary)] mb-1">アカウント作成</h2>
            <p className="text-xs text-[var(--text-muted)] tracking-wider mb-10">無料で始めるAIタスク管理</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="reg-confirm" type={showConfirm ? "text" : "password"} value={confirm}
              ...
              <div className="space-y-3 pt-2">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-xs text-[var(--text-muted)] leading-relaxed">13歳以上です</span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
              ...
              <button type="submit" disabled={loading || !ageConfirmed || !termsAccepted}
            ...
            <p className="text-xs text-[var(--text-subtle)] mt-8 tracking-wide">
              すでにアカウントをお持ちの方は{" "}
              <Link to="/login" className="text-[var(--text-primary)] hover:underline">ログイン</Link>
            </p>
          </div>
        </div>
      </div>
